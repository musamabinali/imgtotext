    ...
    <div>
                        ...
                        <div class="w-full flex flex-wrap flex-row justify-between">
                            <div class="flex flex-wrap">
                                <div>
                                    ...
                                    <span class="text-xs font-semibold text-blue-600 my-2">{{ file.name }}</span>
                                </div>
        ...
        <div>
            <div class="flex flex-wrap shadow-2xl mt-4 rounded-lg p-4" v-if="this.files.length && this.submitted">
                ...
                <div class="md:w-1/2 flex flex-wrap justify-between w-full p-3 my-1" v-for="(file,index) in files" :key="index">
                    <div class="flex flex-wrap flex-row w-full justify-between  gap-2  border-2 border-gray-200 rounded-md p-2">
                        <div class="flex flex-col flex-wrap gap-2">
                            <img :src="'images/word_image.jpg'" class="rounded-lg h-5 my-1 mx-auto object-contain" style="aspect-ratio: 16/9;object-fit: cover;">
                            <span class="text-xs font-semibold text-blue-600 my-1">{{ file.name }}</span>
                        </div>
<!--                        <div class="my-auto">
                            <span class="text-blue-600">{{Math.floor(file.file.size/1024)+"KB"}}</span>
                        </div>-->
                        <div class="my-auto">
                            <a target="_blank" :href="file.dataURL" :download="file.name" class="w-full bg-blue-500 hvoer:bg-blue-700 p-1 mt-1 text-white rounded">Download</a>
                        </div>
                        <div class="my-auto">
    ...
    setup() {
        ...
        const uploadFile = async (file) => {
            ...
            try {
                ...
                const pdf = await pdfjsLib.getDocument(objectURL).promise.catch((error) => {
                    console.error('Error loading PDF:', error);
                    throw new Error('PDF loading failed');
                });

                if (pdf) { // Check if pdf is not undefined
                    // Get the first page of the PDF
                    const firstPage = await pdf.getPage(1);
                    const firstTextContent = await firstPage.getTextContent();
                    const firstPageText = firstTextContent.items.map(item => item.str).join(' ');

                    // Initialize a new Word document with the first page content
                    const doc = new docx.Document({
                        sections: [{
                            properties: {},
                            children: [new docx.Paragraph(firstPageText)],
                        }],
                    });

                    // Loop through the remaining pages of the PDF
                    for (let pageNum = 2; pageNum <= pdf.numPages; pageNum++) {
                        const page = await pdf.getPage(pageNum);
                        const textContent = await page.getTextContent();
                        const pageText = textContent.items.map(item => item.str).join(' ');

                        // Add the remaining pages as new sections
                        doc.addSection({
                            children: [new docx.Paragraph(pageText)],
                        });
                    }

                    // Save the Word document as a Blob
                    const blob = await docx.Packer.toBlob(doc).catch((error) => {
                        console.error('Error creating Word Blob:', error);
                        throw new Error('Word Blob creation failed');
                    });

                    if (blob) { // Check if blob is not undefined
                        // Create a new File object with the Word blob
                        const newFileName = file.name.replace(/\.[^/.]+$/, "") + ".docx";
                        const wordFile = new File([blob], newFileName, {
                        ...
                        setTimeout(() => {
                            ...
                            // Create a local URL for the converted Word file
                            file.dataURL = URL.createObjectURL(wordFile);
                            file.name = newFileName; // Update the file name to reflect the conversion

                            // Cleanup: Revoke the object URL to avoid memory leaks
                            URL.revokeObjectURL(objectURL);
                        }, 500); // Simulated delay to show progress
                    } else {
                ...
                file.uploading = false;
                file.error = "Error converting PDF to Word";
            }
        };
        ...
        const uploadFileOld = async(file) => {
            ...
            formData.append("convert",'to-Docx');
            ...
            try {
                ...
                const response = await axios.post("/file-upload-Docx",formData, config);
                file.uploading = false;
                file.uploaded = true;
                file.uploadProgress=100;
